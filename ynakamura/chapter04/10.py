...
def convert_to_phys(L, S, fs):
    phys_freq = fs / (L + 2)
    phys_time1 = S / fs
    phys_time2 = (S - L) / fs
    return phys_freq, phys_time1, phys_time2
...
phys_freq, phys_time1, phys_time2 = convert_to_phys(L, S, fs)

plt.pcolormesh(np.abs(X))
plt.xticks(
    np.linspace(0, X.shape[1], 5),
    np.linspace(0, X.shape[1] * phys_time1 + phys_time2, 5),
)
plt.yticks(
    np.linspace(0, X.shape[0], 5), np.linspace(0, X.shape[0] * phys_freq, 5).astype(int)
)
plt.show()
